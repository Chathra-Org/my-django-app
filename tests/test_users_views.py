...
from users.views import add_user, update_user
...
class TestUsersViews:
    ...
    def test_update_user_with_valid_data(self):
        ...
        response = client.put("/users/testuser/", data=json.dumps(data), content_type="application/json")
        assert response.status_code == 200
        assert response.json()["message"] == "User testuser updated"
        assert response.json()["email"] == "updated@example.com"

    def test_update_user_with_missing_email(self):
        client = Client()
        data = {}
        response = client.put("/users/testuser/", data=json.dumps(data), content_type="application/json")
        assert response.status_code == 400
    ...
    def test_update_user_with_invalid_json(self):
        client = Client()
        data = "invalid json"
        response = client.put("/users/testuser/", data=data, content_type="application/json")
        assert response.status_code == 400
        assert response.json()["error"] == "Invalid JSON"
